...
def main():
    """
    Description:
        A program to generate multi-axis trapezoidal velocity trajectories.
    ...
    for i, (s_0_i, s_f_i) in enumerate(zip(s_0, s_f)):
        ...
        (_, s_dot, _) = Trapezoidal_Cls.Generate(s_0_i, s_f_i, 0.0, 1.0)

        ax.plot(Trapezoidal_Cls.t, s_dot, '.--', linewidth=1.0, markersize = 3.0, 
                markeredgewidth = 1.5, label=r'$\dot{s}_{%d}(t)$' % (i + 1))

    # Set parameters of the graph (plot).
    ax.set_title(r'Trajectory Trapezoidal Profile: Velocity', fontsize=25, pad=25.0)
    #   Set the x ticks.

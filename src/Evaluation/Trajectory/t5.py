...
def lspb(via,dur,tb):
    ...
    for i in range(0,len(via)-1):
        v_seg[i]=(via[i+1]-via[i])/(15.0 - 0.0 - 2*dur[i])
    print(v_seg)
    ...
    (s, s_dot, s_ddot) = P_Cls.Generate(np.array([pos[-1], v_seg[1], 0.0]), np.array([pos[-1] + v_seg[1]*((T_via[2]-tb[3]) - (T_via[1]+tb[2])), v_seg[1],  0.0]), T_via[1]+tb[2], T_via[2]-tb[3])
    time    = np.concatenate((time, P_Cls.t))
    pos     = np.concatenate((pos, s))
    speed   = np.concatenate((speed, s_dot))

    (s, s_dot, s_ddot) = P_Cls.Generate(np.array([pos[-1], v_seg[1], 0.0]), np.array([via[2] + v_seg[2] * tb[0], v_seg[2],  0.0]), T_via[2]-tb[3], T_via[2]+tb[3])
    time    = np.concatenate((time, P_Cls.t))
    pos     = np.concatenate((pos, s))
    speed   = np.concatenate((speed, s_dot))

    (s, s_dot, s_ddot) = P_Cls.Generate(np.array([pos[-1], v_seg[2], 0.0]), np.array([pos[-1] + v_seg[2]*((T_via[3]-tb[3]) - (T_via[2]+tb[3])), v_seg[2],  0.0]), T_via[2]+tb[3], T_via[3]-tb[3])
    time    = np.concatenate((time, P_Cls.t))
    pos     = np.concatenate((pos, s))
    speed   = np.concatenate((speed, s_dot))

    # ...
    (s, s_dot, s_ddot) = P_Cls.Generate(np.array([pos[-1], v_seg[-1], 0.0]), np.array([via[-1], 0.0,  0.0]), T_via[-1]-tb[-1], T_via[-1]+tb[-1])
    time    = np.concatenate((time, P_Cls.t))
    pos     = np.concatenate((pos, s))
    speed   = np.concatenate((speed, s_dot))
